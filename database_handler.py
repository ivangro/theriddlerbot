...
def load_database(dbname):
	...
	if not 'user_scores' in data.tables:
		...
		field('correct_answers', INTEGER, optional=False, default=0),	# nr of correct answers in total
		field('correct_answers_week', INTEGER, optional=False, default=0)	# nr of correct answers in last week
		))
...
def update_score(data, author_id, author_name):
	...
	if author_id in {row[0]:True for row in data.user_scores.filter('author_id')}:
		...
		week_score = data.user_scores.filter('correct_answers_week', author_id=author_id)[0][0]
		week_score += 1
		data.user_scores.update(author_id, correct_answers_week=week_score)
	else:
		data.user_scores.append(author_id=author_id, author_name=author_name, correct_answers=1, correct_answers_week=1)
	return data
	
def reset_week_scores(data):
	"""
	Reset 'correct_answers_week' to 0 for all users
	"""
	for author_id in data.user_scores.filter('author_id'):
		data.user_scores.update(author_id[0], correct_answers_week=0)
	return data
...
if __name__ == '__main__':
	...
	elements = []
	for a,b,c in data.user_scores.rows():
		elements.append([a,b,c,0])
	print elements
	data.remove('user_scores')
	data.create('user_scores',fields=(
		field('author_id', INTEGER, index=PRIMARY, optional=False ),	# id of twitter user
		field('author_name', STRING(30), index=True, optional=False ),		# name of twitter user
		field('correct_answers', INTEGER, optional=False, default=0),	# nr of correct answers in total
	...
	for row in elements:
		data.user_scores.append(author_id=row[0], author_name=row[1], correct_answers=row[2], correct_answers_week=row[3])
	print data.user_scores.rows()
	"""
	#data = reset_week_scores(data)
	#print data.user_scores.rows()
